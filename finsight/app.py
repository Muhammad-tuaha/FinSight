...
from core.ratio_engine import compute_ratios, statements_complete
...
def analyze_report():
    """Full pipeline: parse → extract → ratios (current + prior) → flags → narrative."""
    logger.info("Incoming file parsing request captured on /api/v1/analyze")
    ...
    try:
        ...
        current_ratios = compute_ratios(current_period)
        prior_ratios = compute_ratios(prior_period) if prior_period else None
